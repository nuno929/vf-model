...
```text
K_i,t = actor-relative access / usability view of K_t
```

`K_i` は主体ごとに別の資源世界を意味しない。また、`K_i` の “slice” は集合論的な部分集合を必ずしも意味しない。情報、権利、信用枠、制度上利用可能な手段等を含むため、共通 K に対する主体 `i` の access / usability relation によって定まる actor-relative view として扱う。
...
`ΔK(S, τ)` は、その仕様 `S` について時間区間 `τ` 内に実現した実資源量の差分（総量変化）を表す。`ΔP_i` は主体 `i` の P に実現した区間差分を表す。
...
`ΔK` はミクロ用・マクロ用に別の型へ分けない。個人の消費・生産で観測される資源差分も、複数主体・長期区間で集約される資源差分も、観測・会計仕様 `S` と時間区間 `τ` が異なる同じ `ΔK` として扱う。**この同型性と会計接続可能性がミクロとマクロの接続点である。**
...
主体 `i` は `K_i` と `P_i` のもとで、自らが生産・消費・交換すると見込む資源ポートフォリオの変化 `E_i[ΔK(S_i, τ)]` を形成し、A を選択する。`S_i` は共通 K 上の主体 `i` の経済的な観測・会計 scope であり、主体ごとの別資源世界を意味しない。

### 会計的予算制約

価格・交換比率等で会計化された主体ごとの期待資源変化が、その主体の予算制約と整合する条件を記述できる。
...
この式は期待値上の会計整合の例であり、state-wise / almost-sure な action feasibility や contingent plan の実行可能性を保証するものではない。
...
同じ資源変化を一貫した会計境界・座標・変換規則で記録することで、主体レベルの資源変化と共通 K の `ΔK` を同じ記法体系で接続できる。`ΔK` の型自体をミクロ／マクロで分ける必要はない。
...
スケールを変えても `ΔK` の型は変えない。変わるのは観測・会計仕様 `S`、時間区間 `τ`、集約規則である。
