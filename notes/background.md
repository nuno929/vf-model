...
現行の field は独立変数ではなく、K / K_i / P_i と projection-specified relations / constraints が A を条件づけ、誘発し、再生産する配置構造を指す。
...
VFT における use-value quantity は、resource stock 自体や technical service potential ではなく、**主体が interval 内に resource を実際に利用・消費し、その体験として ex post に realized した主観的効用量**とする。

physical stock / capability は K として時点観測できる一方、それは use-value quantity そのものではない。

同じ主体・同じ resource・同じ利用量でも、充足状態、順序、文脈、他の経験等によって realized utility は変わりうる。限界効用逓減はその一例である。そのため use-value quantity に一般的な再現性を仮定しない。

この整理により、use-value quantity は point-in-time stock としては定義せず、**実利用後の subjective interval outcome** としてのみ扱う。
...
したがって時点 `t` で参照できる use-value は前区間までの realized flow である。将来区間の use-value はまだ実現しておらず、P_i 上の expectation として表現される。
...
したがって「use-value = flow / exchange-value = stock」という完全な一対一対応は撤回した。ただし **VFT-specific use-value quantity は interval realization に限定し、exchange-value representation は stock / flow の双方を取りうる**という非対称性を維持する。
...
```text
field_t
= configuration of K_t, (K_i,t)_i, (P_i,t)_i, projection-specified relations / constraints
          ↓
        (A_i)_i
          ↓
resource activity / transformation / exchange / learning
          ↓
ΔK / subjective use experience / exchange-value changes
          ↓
